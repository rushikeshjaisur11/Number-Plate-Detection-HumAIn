...
def findPossibleCharsInPlate(imgGrayscale, imgThresh):
    ...
    for contour in contours:                        # for each contour
        possibleChar = utils.PossibleChar.PossibleChar(contour)
...
def findListOfListsOfMatchingChars(listOfPossibleChars):
    ...
    for possibleChar in listOfPossibleChars:                        # for each possible char in the one big list of chars
        ...
        #imgContours = np.zeros((height, width, 3), np.uint8)
        #cv2.drawContours(imgContours, possibleChar.contour, -1, ImageDetect.SCALAR_WHITE)
...
def findListOfMatchingChars(possibleChar, listOfChars):
    ...
    for possibleMatchingChar in listOfChars:                # for each char in big list
        ...
        if (fltDistanceBetweenChars < (possibleChar.fltDiagonalSize * MAX_DIAG_SIZE_MULTIPLE_AWAY) and
            ...
            #cv2.drawContours(imgContours, possibleMatchingChar.contour, -1, ImageDetect.SCALAR_WHITE)
            #cv2.imshow("2b", imgContours)
            #cv2.waitKey(0)
        # end if
    # end for
